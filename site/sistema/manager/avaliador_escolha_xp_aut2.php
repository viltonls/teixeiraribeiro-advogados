...
?>
  ...
  <? for ($i = 0; $i < count($avaliadorList); $i++) {
      ...
      <?  if($avaliador->getSTATUS()=="1" && $cel == $area_aval){
		   ...
		   echo"<br>$cont<br>";
		    ...
		    if($cont > $cont_volta ){
			  if($aval_id != $aval_id_vinc && $trab_id !=  $trab_id_vinc ) {
	             ...
	             if ($trabalho->getID() && $trabalho->getSTATUS() == $trabalho->STATUS_NOVO && $cel == $area_aval) {
		   ...
		   header( "Location:avaliador_escolha_xp_aut.php?trab_id_vinc=$trab_id_vinc&area=$area" ) ;
