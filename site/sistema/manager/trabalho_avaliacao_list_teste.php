...
<div class="structTitle">Listar Avaliações do Trabalho <?=$trab_id?></div>

(<a href="avaliador_escolha_list.php?trab_id=<?=$trab_id?>">Incluir avaliador para este trabalho</a>)
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  ...
  <tr class="gridHeader" style="top:expression(document.getElementById('tC').scrollTop);">
    ...
    <td class="gridHeader">Avaliador</td>

    <td class="gridHeader">Avaliação Geral</td>

    <td class="gridHeader">Nota(s)</td>

    <td class="gridHeader">Data Avaliação</td>
  ...
  <tr>
    ...
    <td class="gridLineEven"><?= $avaliacaoAvaliador->getNOME() ?> (<?= $avaliacaoAvaliador->getLOGIN() ?>)&nbsp;</td>
    ...
    <td class="gridLineEven">
    	...
    	<? $Soma[$i] = number_format($avaliacaoAvaliador->getNOTA_A() + $avaliacaoAvaliador->getNOTA_B() + $avaliacaoAvaliador->getNOTA_C() + $avaliacaoAvaliador->getNOTA_D() + $avaliacaoAvaliador->getNOTA_E() + $avaliacaoAvaliador->getNOTA_F() + $avaliacaoAvaliador->getNOTA_G() + $avaliacaoAvaliador->getNOTA_H() + $avaliacaoAvaliador->getNOTA_I(),2)  ?>
		
		SOMA: <?=$Soma[$i]?>

    </td>

    <td class="gridLineEven"><? if ($avaliacaoAvaliador->getDATA_AVALIADO()!="0000-00-00 00:00:00") echo date("d/m/Y H:m",strtotime($avaliacaoAvaliador->getDATA_AVALIADO())) ?>&nbsp;</td>
    ...
    <!--td class="gridLineEven"><a href="avaliador_edit.php?id=<?= $avaliacaoAvaliador->getAVAL_ID() ?>">Ver&nbsp;Avaliador</a>&nbsp;</td-->
...
<? // calcula média para evento ENGEMA
 if ($eventoAtual->getID()=='6'){
	$Media = 0;
 	for($x=0;$x<count($Soma);$x++){
	 	$Media = $Media + $Soma[$x];
	}
	$Media = $Media / count($Soma);
	echo "MÉDIA: $Media";
 }
 ?>
...
<table border="0" cellpadding="0" cellspacing="4" width="90%">
		...
		<tr>
			...
			<td class="structFilter" colspan="3">
				...
				<select class="structFilterBox" name="area" id="area">
					...
					<option value="1" <?= ($trabalho->getAREA()=="1") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea1PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea2PT()) { ?>

					<option value="2" <?= ($trabalho->getAREA()=="2") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea2PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea3PT()) { ?>

					<option value="3" <?= ($trabalho->getAREA()=="3") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea3PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea4PT()) { ?>

					<option value="4" <?= ($trabalho->getAREA()=="4") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea4PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea5PT()) { ?>

					<option value="5" <?= ($trabalho->getAREA()=="5") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea5PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea6PT()) { ?>

					<option value="6" <?= ($trabalho->getAREA()=="6") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea6PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea7PT()) { ?>

					<option value="7" <?= ($trabalho->getAREA()=="7") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea7PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea8PT()) { ?>

					<option value="8" <?= ($trabalho->getAREA()=="8") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea8PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea9PT()) { ?>

					<option value="9" <?= ($trabalho->getAREA()=="9") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea9PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea10PT()) { ?>

					<option value="10" <?= ($trabalho->getAREA()=="10") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea10PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea11PT()) { ?>

					<option value="11" <?= ($trabalho->getAREA()=="11") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea11PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea12PT()) { ?>

					<option value="12" <?= ($trabalho->getAREA()=="12") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea12PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea13PT()) { ?>

					<option value="13" <?= ($trabalho->getAREA()=="13") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea13PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea14PT()) { ?>

					<option value="14" <?= ($trabalho->getAREA()=="14") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea14PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea15PT()) { ?>

					<option value="15" <?= ($trabalho->getAREA()=="15") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea15PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea16PT()) { ?>

					<option value="16" <?= ($trabalho->getAREA()=="16") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea16PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea17PT()) { ?>

					<option value="17" <?= ($trabalho->getAREA()=="17") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea17PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea18PT()) { ?>

					<option value="18" <?= ($trabalho->getAREA()=="18") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea18PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea19PT()) { ?>

					<option value="19" <?= ($trabalho->getAREA()=="19") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea19PT(); ?></option>

					<? } ?>	    

					<? if ($eventoConfiguracaoXMLAtual->getTrabalhoArea20PT()) { ?>

					<option value="20" <?= ($trabalho->getAREA()=="20") ? "selected" : "";?>><?= $eventoConfiguracaoXMLAtual->getTrabalhoArea20PT(); ?></option>
		...
		<tr>
			...
			<td class="structFilter" colspan="3"><textarea class="structFilterBox" name="resumo" rows="3" cols="120"><?= $trabalho->getOBS()?></textarea></td>
